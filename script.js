...
const objectColors = ['blue', 'white', 'yellow', 'green', 'red', 'orange'];
const targetCountPerColor = 9;
let collectedCounts = {};
...
function createAndShuffleCollectibles() {
    const allCollectibles = [];
    const collectibleSize = 20;
    for (const color of objectColors) {
        for (let i = 0; i < targetCountPerColor; i++) {
            allCollectibles.push(new Collectible(0, 0, collectibleSize, color));
        }
    }

    for (let i = allCollectibles.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [allCollectibles[i], allCollectibles[j]] = [allCollectibles[j], allCollectibles[i]];
    }
    return allCollectibles;
}

function placeCollectiblesForLevel(level, levelCollectibles) {
    const collectibleSize = 20;
    const currentLevelPlatforms = level.platforms;

    for (const collectible of levelCollectibles) {
        let placed = false;
        let attempts = 0;
        while (!placed && attempts < 100) {
            const randomPlatform = currentLevelPlatforms[Math.floor(Math.random() * currentLevelPlatforms.length)];
            const x = randomPlatform.x + Math.random() * (randomPlatform.width - collectibleSize);
            const y = randomPlatform.y - collectibleSize - (Math.random() * 50 + 20);

            if (x >= 0 && x + collectibleSize <= worldWidth && y >= 0 && y + collectibleSize <= canvas.height) {
                collectible.x = x;
                collectible.y = y;
                placed = true;
            }
        ...
        if (!placed) {
            collectible.x = Math.random() * (worldWidth - collectibleSize);
            collectible.y = Math.random() * (canvas.height - collectibleSize - 100) + 50;
        }
    }
...
function loadLevel(levelIndex) {
    ...
    goal = level.goal;
    const levelCollectiblesData = shuffledCollectibles.slice(levelIndex * 9, (levelIndex + 1) * 9);
    const newCollectibles = levelCollectiblesData.map(collectible => new Collectible(0, 0, collectible.width, collectible.color));

    collectibles = placeCollectiblesForLevel(level, newCollectibles);
...
function updateStatisticsDisplay() {
    statsDisplay.innerHTML = '<h2>Collected Objects</h2>';
    objectColors.forEach(color => {
        const count = collectedCounts[color] || 0;
        const item = document.createElement('div');
        item.classList.add('stat-item');

        const colorBox = document.createElement('div');
        colorBox.classList.add('stat-object-color');
        colorBox.style.backgroundColor = color;

        const text = document.createElement('span');
        text.textContent = `${color}: ${count}/${targetCountPerColor}`;
...
function init() {
    ...
    objectColors.forEach(color => {
        collectedCounts[color] = 0;
    });

    // Create and shuffle collectibles
    shuffledCollectibles = createAndShuffleCollectibles();

    // --- DEBUG CODE START ---
    const colorCounts = {};
    shuffledCollectibles.forEach(c => {
        if (!colorCounts[c.color]) {
            colorCounts[c.color] = 0;
        }
        colorCounts[c.color]++;
    });
    console.log("Initial collectible color counts:", colorCounts);
    // --- DEBUG CODE END ---
...
function gameLoop() {
    ...
    if (gameWon) {
//        window.location.href = 'rubiks_cube.html';
        window.location.href = 'rubik-js-master/index.html';
        return;
    }
    ...
    collectibles.forEach((collectible, index) => {
        ...
        if (checkCollision(player, collectible)) {
            collectedCounts[collectible.color]++;
    ...
    if (checkCollision(player, goal)) {
        // Check if all collectibles in the current level are collected
        if (collectibles.length === 0) {
            currentLevel++;
            if (currentLevel >= levels.length) {
                // Check global win condition only at the end of the last level
                let allGlobalCollected = true;
                objectColors.forEach(color => {
                    if (collectedCounts[color] < targetCountPerColor) {
                        allGlobalCollected = false;
                    }
                });

                if (allGlobalCollected) {
                    gameWon = true;
                } else {
                    // If all level objects collected but not global, maybe a message?
                    console.log("You completed all levels, but didn't collect all global objects!");
                    gameWon = true; // Still win, but maybe a different message or ending
                }
            } else {
                loadLevel(currentLevel);
            }
        } else {
            // Provide feedback to the player that they need to collect more in this level
            console.log("Collect all objects in this level before proceeding!");
        }
    }
