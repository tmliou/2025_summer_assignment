...
function spawnCollectiblesForLevel(levelIndex) {
    const level = levels[levelIndex];
    const newCollectibles = [];
    const collectibleSize = 20; // Size of the collectible object
    const numCollectiblesPerLevel = 9; // Each level has 9 objects

    // Get all platforms for the current level
    const currentLevelPlatforms = level.platforms;

    for (let i = 0; i < numCollectiblesPerLevel; i++) {
        const randomColor = objectColors[Math.floor(Math.random() * objectColors.length)];
        
        // Try to place on a random platform
        let placed = false;
        let attempts = 0;
        while (!placed && attempts < 100) { // Limit attempts to prevent infinite loops
            ...
            const y = randomPlatform.y - collectibleSize - (Math.random() * 50 + 20); // Place above platform, in the air

            // Check if the collectible is within canvas bounds
            if (x >= 0 && x + collectibleSize <= worldWidth && y >= 0 && y + collectibleSize <= canvas.height) {
                newCollectibles.push(new Collectible(x, y, collectibleSize, randomColor));
                placed = true;
            }
            attempts++;
        }
        // If after many attempts, it can't be placed on a platform, place it randomly in the air
        if (!placed) {
            const x = Math.random() * (worldWidth - collectibleSize);
            const y = Math.random() * (canvas.height - collectibleSize - 100) + 50; // Avoid very top/bottom
            newCollectibles.push(new Collectible(x, y, collectibleSize, randomColor));
        }
    }
...
function loadLevel(levelIndex) {
    ...
    goal = level.goal;
    collectibles = spawnCollectiblesForLevel(levelIndex); // Call the new function here
